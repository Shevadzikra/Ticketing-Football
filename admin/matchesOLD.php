...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    if (isset($_POST['add_match'])) {
        ...
        try {
            $stmt = $pdo->prepare("INSERT INTO matches (team_home, team_away, match_date, stadium, description) 
                                  VALUES (?, ?, ?, ?, ?)");
            $stmt->execute([$team_home, $team_away, $match_date, $stadium, $description]);
            
            $_SESSION['success'] = 'Pertandingan berhasil ditambahkan';
            header("Location: matches.php");
            exit;
        } catch (PDOException $e) {
            $_SESSION['error'] = 'Gagal menambahkan pertandingan: ' . $e->getMessage();
        }
        ...
        try {
            ...
            $_SESSION['error'] = 'Gagal memperbarui pertandingan: ' . $e->getMessage();
        }
    }
}
                            ...
                            <tr class="<?= $is_past ? 'bg-yellow-50' : '' ?>">
                                ...
                                <td class="px-6 py-4 whitespace-nowrap">
                                    <span class="px-2 py-1 text-xs bg-red-200 text-red-800 rounded">
                                        <i class="fas fa-times mr-1"></i>Tiket Habis
                                    </span>
                                </td>
                                <td class="px-6 py-4 whitespace-nowrap">
                                    <button onclick="openEditModal(<?= htmlspecialchars(json_encode($match)) ?>)" 
                                    ...
                                    <a href="./tickets/create_tickets.php?match_id=<?= $match['id'] ?>" 
                                       class="text-purple-600 hover:text-purple-900 ml-3"
                                       title="Tambahkan Tiket Baru">
